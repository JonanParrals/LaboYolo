...
class ObjectDetectorHelper(
    ...
    // Umbral de confianza minimo para contar una deteccion (0.5 = 50%). Subelo si sobreestima
    // personas (mas falsos positivos), bajalo si le cuesta detectar a personas reales.
    private val confThreshold: Float = 0.5f,
    private val iouThreshold: Float = 0.45f
) {
    companion object {
        // Indice de la clase "person" en el orden estandar de clases COCO usado por YOLOv8.
    ...
    fun detect(originalBitmap: Bitmap): List<BoundingBox> {
        ...
        try {
            ...
            // NCHW filling: el tensor de entrada del modelo exportado es [1,3,320,320] (canales primero)
            for (c in 0 until 3) {
    ...
    private fun decodeOutput(origWidth: Int, origHeight: Int): List<BoundingBox> {
        ...
        for (i in 0 until numBoxes) {
            // Solo nos interesa la clase "person": ignoramos el resto de las 80 clases COCO
            // para no contar sillas, autos, etc. como si fueran personas.
            ...
            if (personScore > confThreshold) {
                ...
                // Si las coordenadas parecen normalizadas (0..1), escalarlas a pixeles del modelo (0..320)
                ...
                result.add(BoundingBox(
                    ...
                    personScore, PERSON_CLASS_ID, "person"
                ))
            }
        }
